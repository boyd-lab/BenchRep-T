...
Each repertoire is treated as a bag of sequences. Frozen ESM-2, TCR-BERT, or
TCR-VALID representations are optionally combined with V/J gene embeddings,
then aggregated by Gated Attention MIL.
...
from models.ensemble_abmil_pretrained import ABMIL
...
class ABMILEvaluator:
    ...
    def __init__(
        ...
        Args:
            ...
            embedding_dim_aa: CNN API compatibility option; unused.
            embedding_dim_genes: Learned V/J gene embedding dimension.
            kernel: CNN API compatibility option; unused.
            conv_units: CNN API compatibility option; unused.
    ...
    def run_cross_validation(self, metadata_path, target_disease, data_dir,
        ...
        base_method = (
            f'ABMIL_{encoder_label}_CovAdj'
            if covariate_adjust else f'ABMIL_{encoder_label}'
        )
        ...
        for test_fold in outer_test_folds(n_folds):
            ...
            self.model = ABMIL(
                ...
                tcrvalid_repo=self.tcrvalid_repo,
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ABMIL disease classification with a frozen pretrained encoder"
    )
    ...
    parser.add_argument('--embedding_dim_aa', type=int, default=64,
                        help='Compatibility option; unused by pretrained encoders')
    ...
    parser.add_argument('--features', type=str, default='full',
                        choices=['full', 'cdr3_only'],
                        help="Which features to use: 'full' (CDR3 + V/J genes), "
                             "or 'cdr3_only' (CDR3 sequence only). Default: full")
    parser.add_argument('--sequence_encoder', required=True,
                        choices=['esm2', 'tcrbert', 'tcrvalid'],
                        help='Frozen pretrained per-sequence encoder.')
    parser.add_argument('--pretrained_model', default=None,
                        help='Optional Hugging Face model ID/path (ESM-2 or TCR-BERT) '
                             'or packaged TCR-VALID model name.')
    parser.add_argument('--pretrained_batch_size', type=int, default=256,
                        help='Frozen encoder inference batch size (default: 256).')
    parser.add_argument('--embedding_cache_dir', required=True,
                        help='Directory for reusable float16 per-sequence embeddings.')
    parser.add_argument('--tcrvalid_repo', default=None,
                        help='Official TCR-VALID checkout; defaults to ../tcrvalid.')
    parser.add_argument('--include_feature_baselines', action='store_true',
                        help='Unsupported compatibility option for this entry point')
    ...
    if args.include_feature_baselines:
        raise ValueError(
            "--include_feature_baselines is not supported by the pretrained entry point"
        )
    ...
    def make_evaluator(feature):
        return ABMILEvaluator(
            ...
            pretrained_batch_size=args.pretrained_batch_size,
            embedding_cache_dir=args.embedding_cache_dir,
            tcrvalid_repo=args.tcrvalid_repo,
        )
