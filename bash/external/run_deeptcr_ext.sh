...
# Run DeepTCR on external/cohort-merged datasets.
...
LOGDIR=${REPO_ROOT}/logs/deeptcr_ext
  ...
  case "$1" in
    RA)
      DATASET_ARGS=(
        ...
        --output_csv "${RESULTS}/deeptcr_2021_ext_RA_classification.csv"
      )
      ;;
    T1D)
      DATASET_ARGS=(
...
run_dataset() {
  ...
  local gpu="${2:-}"
  local run_ts="$3"
  local log="${LOGDIR}/deeptcr_ext_${dataset}_${run_ts}.log"
  ...
  echo "[$(date +%T)] start ${dataset} DeepTCR -> ${log}"
  {
    echo "[$(date +%T)] start ${dataset} DeepTCR"
    if [ -n "$gpu" ]; then
      echo "Using CUDA_VISIBLE_DEVICES=${gpu}"
      export CUDA_VISIBLE_DEVICES="$gpu"
    fi
    cd "${REPO_ROOT}"
    python -u -m evals.deeptcr_2021_disease_classification \
      "${DATASET_ARGS[@]}" \
      --results_dir "${RESULTS}/deeptcr_ext" \
      --batch_size "$BATCH_SIZE" \
      --device 0 \
      "${extra_flags[@]}"
    status=$?
    echo "[$(date +%T)] done ${dataset} DeepTCR (exit ${status})"
    return $status
  } >"$log" 2>&1
}
...
if $PARALLEL; then
  ...
  for i in "${!pids[@]}"; do
    if ! wait "${pids[$i]}"; then
      echo "Dataset ${labels[$i]} failed; see ${LOGDIR}/deeptcr_ext_${labels[$i]}_${RUN_TS}.log" >&2
      status=1
    fi
  done
  exit $status
